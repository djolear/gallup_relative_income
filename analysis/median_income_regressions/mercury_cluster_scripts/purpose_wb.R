...
pur_regression_function <- function(median_income_var_name, dfg) {
...
master_function <- function(current_year, dfg) {
  ...
  res <-
    foreach(i = 1:length(med_inc_vars), .combine = cbind, .packages = c("tidyverse", "doParallel", "lme4", "broom"))%dopar%{
      pur_regression_function(med_inc_vars[i], dfg)
    }
  
  # res <- 
  #   future_map_dfr(.x = med_inc_vars, .f = pur_regression_function, dfg = dfg)
  
  write_csv(res, paste0("/project/ourminsk/gallup/results/regression/purpose_mi_", dfg$year[1], ".csv"))
