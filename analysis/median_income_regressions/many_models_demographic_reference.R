...
models %>% dplyr::select(mi_var, `scale(mi_value)`) %>% mutate(mi_value = abs(`scale(mi_value)`)) %>% ggplot(aes(fct_reorder(mi_var, mi_value), mi_value)) + geom_col() + coord_flip() + facet_grid(. ~ outcome_key)
