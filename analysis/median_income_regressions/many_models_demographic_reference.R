df <- 
  dfg_rs %>%
  sample_n(500000) %>% 
  gather(outcome_key, value, c("fruits_veggies_scale", "bmi_scale", "ladder_now_scale"))
...
models <- many_models_demographic_reference_w_covariates(c("fruits_veggies_scale", "bmi_scale", "ladder_now_scale"), dfg_rs)

models

models %>% 
  dplyr::select(mi_var, `scale(mi_value)`) %>% 
  mutate(mi_value = abs(`scale(mi_value)`)) %>% 
  ggplot(aes(fct_reorder(mi_var, mi_value), mi_value)) + 
  geom_col() + 
  coord_flip() + 
